...
class SFTTrainer(Trainer):
    ...
    def _prepare_models(self) -> None:
        ...
        # [wjh]在这里，load_diffusion_models，将self.transformer初始化
    ...
    def _prepare_dataset(self) -> None:
        ...
        for config in dataset_configs:
            ...
            logger.info(f"Dataset class: {dataset.__class__.__name__}")
        ...
        while True:
            ...
            for index, (key, artifact) in enumerate(list(artifacts.items())):
                ...
                if isinstance(artifact, data.ImageArtifact):
                    ...
                    all_processes_artifacts.append(wandb.Video(output_filename, caption=PROMPT, format='mp4'))
    ...
    def _prepare_data(
        ...
        if not self.args.enable_precomputation:
            ...
            condition_iterator = preprocessor.consume(
                "condition",
                components=condition_components, # condition_components, 很重要，包含输入到
                data_iterator=data_iterator,
                generator=self.state.generator, # 随机数生成器
                cache_samples=True, # 是否将处理过的样本缓存到内存（加速后续复用）
            )
