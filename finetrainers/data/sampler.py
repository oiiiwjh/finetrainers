...
class ResolutionSampler:
        ...
        for dict_item in dict_items:
            for key in self.dim_keys.keys():
                if key in dict_item.keys():
                    self._chosen_leader_key = key # generally [latent]
                    if not torch.is_tensor(dict_item[key]): # latent must be a tensor
                        raise ValueError(f"Leader key {key} must be a tensor")
                    num_observed += 1
        if num_observed > 1: # more than one leader key found
            raise ValueError(
                f"Only one leader key is allowed in provided list of data dictionaries. Found {num_observed} leader keys"
            )
        if self._chosen_leader_key is None: # no leader key found[no latent key found]
    ...
    def _update_buckets(self, *dict_items: Dict[Any, Any]) -> None:
        # consider the chosen_leader_key's shape, segment different shape into buckets
        chosen_value = [
            dict_item[self._chosen_leader_key] # latent shape 
            for dict_item in dict_items
        ...
        chosen_value = chosen_value[0] # (2, 3, 4) tensor
        dims = tuple(chosen_value.size(x) for x in self.dim_keys[self._chosen_leader_key]) # get (2,3,4) dimensions of latent
